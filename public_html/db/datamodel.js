...
class DataModel
{
   ...
   * @param {String} msg.cmd - Command type from DataModel.commandTypes
  ...
  {
    switch (msg.cmd) {
      case DataModel.commandTypes.open:
        await this.openConnection(msg);
        break;

      case DataModel.commandTypes.close:
        await this.closeConnection(msg);
        break;

      case DataModel.commandTypes.execute:
        return await this.execute(msg);

      case DataModel.commandTypes.begin:
        await this.beginTransaction(msg);
        break;

      case DataModel.commandTypes.commit:
        await this.commitTransaction(msg);
        break;

      case DataModel.commandTypes.rollback:
        await this.rollbackTransaction(msg);
        break;

      case DataModel.commandTypes.listTables:
        return await this.listTables(msg);

      case DataModel.commandTypes.listTablePrimaryKeys:
        return await this.listTablePrimaryKeys(msg);

      case DataModel.commandTypes.listTableColumns:
        return await this.listTableColumns(msg);

      case DataModel.commandTypes.listTableForeignKeys:
        return await this.listTableForeignKeys(msg);

      case DataModel.commandTypes.ping:
        this.ping(msg);
        break;
      ...
      if (sql.slice(i, i + parName.length) === parName) {
        ...
        if (beforeOk && afterOk) {
          ...
          // Use improved quoteString for string values
            ...
            par = DataModel.quoteString(par.toISOString());
          }
          else {
            // For other types, convert to string and escape
            par = DataModel.quoteString(String(par));
          }
          //
          result += par;
          i += parName.length;
   ...
   * @param {Server} msg.server - Server instance associated with this connection
   * @throws {Error} Throws error if driver module not found or connection fails
   */
  async openConnection(msg)
  {
    // Load module
    if (!this.loadModule())
   ...
   */
  async listTablePrimaryKeys(msg)
  {
    let conn = this.connections[msg.cid];
    if (!conn)
      throw new Error("Connection closed");
   ...
   */
  async listTableForeignKeys(msg)
  {
    let conn = this.connections[msg.cid];
    if (!conn)
      throw new Error("Connection closed");
   ...
   * @param {Server} server - Server instance that has disconnected
   */
   ...
   * @param {Server} [server] - Optional server instance to filter connections (if provided, only closes connections to this server)
