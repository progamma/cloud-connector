...
// Lazy require to break the FS<->Directory circular dependency at module load time.
let FS;
 ...
 *
 * @param {FS} fs - The file system instance that manages this directory
 ...
 * @property {FS} fs - Reference to the file system instance
 */
class Directory
{
  constructor(fs, path)
  ...
  /**
   * Sets the directory path, automatically normalizing it.
   * @type {String}
   */
  set path(newValue)
  {
   ...
   * @see Directory#exists - To check if directory exists before creation
   */
  async create()
  {
   ...
   * @param {String|Directory} newDir - New directory name or path (string for rename, Directory object for move)
   ...
   * @returns {Promise<Directory>} The new Directory object representing the copied directory
   ...
   * @returns {Promise<Array<File|Directory>>} Array of File and Directory objects found in the directory
   ...
   * @returns {Promise<File>} File object representing the created ZIP file
   * @throws {Error} If compression fails or insufficient disk space
   * @see File#unzip - To extract the created ZIP file
   */
  async zip()
  {
