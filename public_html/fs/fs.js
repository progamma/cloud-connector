...
{
  /**
   * Supported permission levels for shared file systems
   * @enum {String}
   */
  static permissions = {
    read: "r",
  ...
  constructor(parent, config)
  {
    ...
    this.permissions = this.permissions || FS.permissions.read;
    //
    // Files opened
    this.files = {};
  }
   ...
   * @returns {File} File object for performing file operations
   */
  file(path, id)
  {
   ...
   * @returns {Directory} Directory object for performing directory operations
   */
  directory(path)
  {
   ...
   * @returns {Url} URL object for performing remote file operations
   */
  url(url)
  {
   ...
   * @param {Server} server - server disconnected
   */
...
//      throw new Error(`Error closing file '${f.path}': ${error}`);
   ...
   * @param {File|Directory} obj - File or directory object to get absolute path for
