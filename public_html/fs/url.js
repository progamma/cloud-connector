...
class Url
{
   ...
   * @param {File} [file] - Target file for download (auto-created if not provided)
   * @param {Object} options - Request options (headers, timeout, etc.)
  ...
  async download(file, options)
  {
    ...
    if (!file) {
      let path = `tempDownloadedFile_${Utils.generateUID36()}`;
   ...
   */
  onUploadProgress(bytesSent, total)
  {
    return true;
  }
   ...
   */
  onDownloadProgress(bytesTransfered, total)
  {
    return true;
  }
}
